...
export default function Home() {
  ...
  useEffect(() => {
    const loadPlans = async () => {
      try {
        const res = await fetch("https://api.atpuae.com/v1/api/subscriptions?active=true&limit=3");
  ...
  const uploadKyc = async (file: File): Promise<string | null> => {
    try {
      ...
      const res = await fetch("https://api.atpuae.com/v1/api/upload/kyc-document", { method: "POST", body: fd });
      const json = await res.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("https://api.atpuae.com/v1/api/vendors/apply", {
        method: "POST",
